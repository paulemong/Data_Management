...
obungfemale <- filter(kisumu_main_vendor, slum == "Obunga" & gender =="Female")

obungmultifemale <- filter(kisumu_main_vendor, slum == "Obunga" & gender =="Multiple females")

#Example:  selects rows where the animal age is less than 12 weeks
data <- filter(Animal_Data, age < 12)

data_age <- filter(Animal_Data, age > 10)

newdata <- filter(Animal_Data, age < 12 & age > 10)
...
newdata5 <- select(kisumu_main_vendor, -subcounty,-ward,-slum)
newdata51 <- select(kisumu_main_vendor, -c(subcounty, ward, slum))
...
table(kisumu_main_vendor$subcounty)  # Gives the categories of subcounty
table(kisumu_main_vendor$ward)  # Gives the categories of ward
transform(table(kisumu_main_vendor$ward))
table(Animal_Data$breed) # Gives the categories of breed
table(Animal_Data$Species) # Gives the categories of species
...
any(is.na(kisumu_main_vendor)) # Returns TRUE if any value in the dataframe is missing.

sum(is.na(kisumu_main_vendor)) # Gives the total number of missing values in the entire dataframe.

colSums(is.na(kisumu_main_vendor)) # Shows how many missing values are in each column.

kisumu_main_vendor[!complete.cases(kisumu_main_vendor), ] # Returns all rows that contain at least one missing value.
...
# The p-value is 2.2e-16 which is less than significance level of alpha = 0.05.
# Thus, we conclude that age is not normally distributed.
...
agriculture1 <- read_csv("agriculture1.csv")
agriculture2 <- read_csv("agriculture2.csv")
agriculture_townA <- read_csv("agriculture_townA .csv")
agriculture_townB <- read_csv("agriculture_townB.csv")


#check for duplicates
duplicated(agriculture) # checks if there are duplicate observations/rows in the dataset
table(duplicated(agriculture)) # gives the number of duplicates in the dataset

duplicated(agriculture1)
table(duplicated(agriculture1)) # there are 19 duplicates
...
cleandata <- distinct(agriculture1) # The function distinct() removes the duplicate observations/rows
# or
cleandata2 <- unique(agriculture1) # he function unique() removes the duplicate observations/rows
...
duplicated(agric_AB)
table(duplicated(agric_AB)) # There is 1 duplicated observation/row
